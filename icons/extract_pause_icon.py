#!/usr/bin/env python3
"""Extract the pause button sticker from pauseIcon.png."""
from PIL import Image
import numpy as np
from scipy.ndimage import label
from collections import deque

src = Image.open("Images/pauseIcon.png").convert("RGBA")
...
dark_bg.save("icons/_verify_pause_dark.png")

sizes = [16, 24, 32, 48, 64, 128, 256]
frames = [out.resize((s, s), Image.LANCZOS) for s in sizes]
frames[0].save(
    "icons/pauseIconic.ico",
    format="ICO",
    sizes=[(s, s) for s in sizes],
    append_images=frames[1:],
)
print(f"Extracted pause icon {cw}x{ch_px} -> 1024x1024 png + ico")
