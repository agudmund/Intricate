#!/usr/bin/env python3
"""Extract the return-arrow sticker from Images/Stickers/Return.png.

First authored for AudioNode's loop-off toggle (direct playback, no loop),
the sticker ended up reused across the app as a general-purpose return /
back-arrow symbol. The source already carries its own alpha channel, so
the pipeline reduces to: largest-component cleanup → white-matte defringe
on semi-transparent edges → square + 1024 resize → PNG + multi-res ICO.
"""
from PIL import Image
import numpy as np
from scipy.ndimage import label

src = Image.open("Images/Stickers/Return.png").convert("RGBA")
...
out.save("icons/returnIconic.png")
...
out.save("icons/returnIconic.ico", format="ICO", sizes=[(s, s) for s in sizes])
print(f"Extracted return sticker {cw}x{ch_px} -> 1024x1024 png + ico")
